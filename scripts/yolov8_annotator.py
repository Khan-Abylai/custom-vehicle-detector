...
out_folder = '/mnt/custom_car_dataset'

image_folder = '/europe_data/albania'

main_dir_name = os.path.basename(image_folder)

images = np.array(glob(osp.join(image_folder, "**", "*.jpg")))
...
for idx, path in enumerate(images):
    ...
    output_folder = Path(osp.join(out_folder, main_dir_name, osp.basename(osp.dirname(path))))
    if not output_folder.exists():
        output_folder.mkdir(exist_ok=True, parents=True)

    out_image_path = osp.join(output_folder, osp.basename(path))
    shutil.copy(path, out_image_path)
    np.savetxt(out_image_path.replace(".jpg", ".cb"), res_boxes)
